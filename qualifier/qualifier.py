...
class Table:
    ...
    def __post_init__(self):
        ...
        self.__str_rows : List[List[str]] = [[str(cell) for cell in row] for row in self.__rows]
    ...
    def __make_and_add_row(self, border_left: str, join_delim: str, border_right: str, row_data: List[Any], fill_char: str = ' ') -> None:
        if self.__centered:
            adjusted_cells = map(lambda cell, size: f'{fill_char}{cell:^{size}}{fill_char}', row_data, self.__max_size)
        else:
            adjusted_cells = map(lambda cell, size: f'{fill_char}{cell:<{size}}{fill_char}', row_data, self.__max_size)
    ...
    def make(self) -> None:
        horizontal_bars = list(map(lambda size: Border.HC * (size + 2), self.__max_size))
